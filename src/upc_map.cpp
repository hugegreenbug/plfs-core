...
int populate_file_view(upc_file_view **file_view, Plfs_fd *pfd) {
    ...
    *file_view = (upc_file_view *) malloc(sizeof(upc_file_view));
    ...
    ret = plfs_getxattr(pfd, &(*file_view)->count, keys[0].c_str(), sizeof(uint32_t));
    if (ret) {
    ...
    (*file_view)->block_lengths = (uint32_t *) malloc(sizeof(uint32_t) * (*file_view)->count);
    ...
    (*file_view)->block_displacements = (uint32_t *) malloc(sizeof(uint32_t) * (*file_view)->count);
    ...
    (*file_view)->types = (uint32_t *) malloc(sizeof(uint32_t) * (*file_view)->count);
    ret = plfs_getxattr(pfd, (*file_view)->types, keys[3].c_str(), 
                        sizeof(uint32_t) * (*file_view)->count);
    if (ret) {
        mlog(PLFS_DBG, "Could not retrieve value for key: %s", 
    ...
    ret = plfs_getxattr(pfd, &(*file_view)->elem_type, keys[4].c_str(), sizeof(uint32_t));
    if (ret) {
        mlog(PLFS_DBG, "Could not retrieve value for key: %s", 
             keys[0].c_str(), __FUNCTION__);
        err = 1;
...
int write_file_view(upc_file_view **file_view, Plfs_fd *pfd) {
    ...
    ret = plfs_setxattr(pfd, &(*file_view)->count, keys[0].c_str(), sizeof(uint32_t));
    if (ret) {
    ...
    ret = plfs_setxattr(pfd, (*file_view)->block_lengths, keys[1].c_str(), 
                        (*file_view)->count * sizeof(uint32_t));
    if (ret) {
    ...
    ret = plfs_setxattr(pfd, (*file_view)->types, keys[3].c_str(), 
                        (*file_view)->count * sizeof(uint32_t));
    if (ret) {
        mlog(PLFS_DBG, "In %s: Error writing upc object type\n", 
             __FUNCTION__);
    ...
    ret = plfs_setxattr(pfd, &(*file_view)->elem_type, keys[4].c_str(), 
                        sizeof(uint32_t));
    if (ret) {
        mlog(PLFS_DBG, "In %s: Error writing upc object size\n", 
             __FUNCTION__);
